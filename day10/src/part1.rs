...
pub fn main() {
    ...
    let mut hashmap: HashMap<(usize, usize), HashSet<(usize, usize)>> = HashMap::new();
    let mut start = traverse_stack[traverse_stack.len() - 1];
    ...
    while let Some(pos) = traverse_stack.pop() {
        ...
        if val == '9' {
            // don't include previous visited 9's
            let entry = hashmap.entry(start).or_insert_with(|| HashSet::new());
